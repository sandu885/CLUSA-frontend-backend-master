...
class Section02 extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.userName === null) {
      ...
      console.warn('section 3 logedin', this.state.logedin);
    } else {
      axios.post(
        getSectionInputAPI,
        ...
        if (response.data.message === 'Successfully get application content') {
          console.warn('section 3 get information successfully!');
          this.setState({
            getS3q1: response.data.content['1'],
            getS3q2: response.data.content['2'],
          });
        } else if (response.data.message === 'No saved application for this section') {
          console.warn('no saved history');
        } else {
          console.warn('section 3 Failed to save');
        }
        console.warn(response.data);
      }).catch((error) => {
        console.warn('section 3 error.response', error.response);
  ...
  clickSaveBtn = () => {
    ...
    console.warn('============== section 3 save data', this.state);
    const finalS3q1 = this.state.s3q1 === null ? this.state.getS3q1 : this.state.s3q1;
    const finalS3q2 = this.state.s3q2 === null ? this.state.getS3q2 : this.state.s3q2;

    const formData = new FormData();
    formData.append('sessionToken', this.state.sessionToken);
    formData.append('programType', '0');
    formData.append('sectionIndex', '3');
    ...
    axios.post(
      ...
      if (response.data.message === 'Successfully save section content') {
        ...
        document.getElementById('internship-s3').reset();
        window.location.reload();
      } else {
        console.warn('section 3 Failed to save');
        alert('section 3 Failed to save, please try again');
      }
  ...
  render() {
    ...
    if (this.state.shouldRedirectToNext === true) return <Redirect to="/internship-application-section04" />;
    if (this.state.shouldRedirectToPrevious === true) return <Redirect to="/internship-application-section02" />;
          ...
          { saveSuccess ? (
            <MDBContainer className="grey darken-3 p-3">
              <MDBNotification
                ...
                title="SECTION 3. Internship Leaders Team"
                    ...
                    <MDBCardBody className="mx-4">
                      <div className="text-left">
                        ...
                        <MDBRow className="mbottom0Imp">
                          ...
                          >
                            <h5>
                              <strong>SECTION 3. Internship Leaders Team</strong>
                            </h5>
                          </MDBCol>
                        ...
                        The typical team has at least two leaders, an Internship Organizer and an Internship Trainer.
                        </p>
                      </div>
                      {/* ----------- main questions ----------- */}
                      <div className="text-left">
                        ...
                        >
                          ...
                          >
                            3.1.
                            <span className="redColor">* </span>The Internship Organizer leads in building relationships for internship placements in elected office/government agency internships, galvanizes community financial support, and organizes the internship graduation celebration. Please provide name and a brief profile of the Internship Organizer.
                          </label>
                          <textarea
                            id="internship-s3-q1"
                            className="form-control"
                            rows="5"
                            defaultValue={this.state.getS3q1}
                            onChange={(e) => (this.setState({ s3q1: e.target.value }))}
                          />
                          ...
                          >
                            3.2.
                            <span className="redColor">* </span>The Internship Trainer leads in organizing training and mentoring of student interns. This person is very skilled at relating to iGeneration youths born after the 2000s. Please provide name and a brief profile of the Internship Trainer.
                          </label>
                          <textarea
                            id="internship-s3-q3"
                            className="form-control"
                            rows="5"
                            defaultValue={this.state.getS3q2}
                            onChange={(e) => this.setState({ s3q2: e.target.value })}
                          />
                        </form>
                      </div>
                      {/* ----------- button group ----------- */}
