...
class Section08 extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    if (this.state.userName === null) {
      ...
      console.warn('section 8 logedin', this.state.logedin);
    } else {
      axios.post(
        getSectionInputAPI,
        ...
        if (response.data.message === 'Successfully get application content') {
          console.warn('section 8 get information successfully!');
          this.setState({
            getS8q1: response.data.content['1'],
          });
        } else if (response.data.message === 'No saved application for this section') {
          console.warn('no saved history');
        } else {
          console.warn('section 8 Failed to save');
        }
        console.warn(response.data);
      }).catch((error) => {
        console.warn('section 8 error.response', error.response);
  ...
  clickSaveBtn = () => {
    ...
    console.warn('============== section 8 save data ================', this.state);
    const finalS8q1 = this.state.s8q1 === null ? this.state.getS8q1 : this.state.s8q1;

    const formData = new FormData();
    formData.append('sessionToken', this.state.sessionToken);
    formData.append('programType', '0');
    formData.append('sectionIndex', '8');
    ...
    axios.post(
      ...
      if (response.data.message === 'Successfully save section content') {
        ...
        document.getElementById('internship-s8').reset();
        window.location.reload();
      } else {
        console.warn('section 8 Failed to save');
        alert('section 8 Failed to save, please try again');
      }
  ...
  render() {
    ...
    if (this.state.shouldRedirectToNext === true) return <Redirect to="/internship-application-section09" />;
    if (this.state.shouldRedirectToPrevious === true) return <Redirect to="/internship-application-section07" />;
          ...
          { saveSuccess ? (
            <MDBContainer className="grey darken-3 p-3">
              <MDBNotification
                ...
                title="SECTION 8. Graduation Ceremony Plan"
                    ...
                    <MDBCardBody className="mx-4">
                      <div className="text-left">
                        ...
                        <MDBRow className="mbottom0Imp">
                          <MDBCol md="10">
                            <h5>
                              <strong>SECTION 8. Graduation Ceremony Plan</strong>
                            </h5>
                          </MDBCol>
                      ...
                      <div className="text-left">
                        ...
                        >
                          ...
                          >
                            8.1.
                            <span className="redColor">* </span>Graduation ceremony is an very important opportunity for community-building and seeking financial support, human resource etc.. Please share your graduation ceremony plan:
                          </label>
                          <textarea
                            id="internship-s8-q1"
                            className="form-control"
                            rows="8"
                            defaultValue={this.state.getS8q1}
                            onChange={(e) => this.setState({ s8q1: e.target.value })}
                          />
                        </form>
                      </div>
                      {/* ----------- button group ----------- */}
