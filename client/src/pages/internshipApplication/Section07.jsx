...
class Section07 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      s7q1: null,
  ...
  componentDidMount() {
    ...
    if (this.state.userName === null) {
      ...
      console.warn('section 7 logedin', this.state.logedin);
    } else {
      axios.post(
        getSectionInputAPI,
        ...
        if (response.data.message === 'Successfully get application content') {
          console.warn('section 7 get information successfully!');
          this.setState({
            getS7q1: response.data.content['1'],
          });
        } else if (response.data.message === 'No saved application for this section') {
          console.warn('no saved history');
        } else {
          console.warn('section 7 Failed to save');
        }
        console.warn(response.data);
      }).catch((error) => {
        console.warn('section 7 error.response', error.response);
  ...
  clickSaveBtn = () => {
    ...
    console.warn('============== section 7 save data', this.state);
    const finalS7q1 = this.state.s7q1 === null ? this.state.getS7q1 : this.state.s7q1;

    const formData = new FormData();
    formData.append('sessionToken', this.state.sessionToken);
    formData.append('programType', '0');
    formData.append('sectionIndex', '7');
    ...
    axios.post(
      ...
      if (response.data.message === 'Successfully save section content') {
        ...
        document.getElementById('internship-s7').reset();
        window.location.reload();
      } else {
        console.warn('section 7 Failed to save');
        alert('section 7 Failed to save, please try again');
      }
      // console.warn(this.state.responseMessage);
    }).catch((error) => {
      console.warn('section 7 error.response', error.response.data);
      if(error.response !== null && error.response !== undefined) {
        if( error.response.data !== null && error.response.data !== undefined ) {
          if (error.response.data.message === 'sessionToken expired' || error.response.data.message === 'No sessionToken') {
            localStorage.clear();
  ...
  render() {
    ...
    if (this.state.shouldRedirectToNext === true) return <Redirect to="/internship-application-section08" />;
    if (this.state.shouldRedirectToPrevious === true) return <Redirect to="/internship-application-section06" />;
          ...
          { saveSuccess ? (
            <MDBContainer className="grey darken-3 p-3">
              <MDBNotification
                ...
                title="SECTION 7. Student Training Plan"
                    ...
                    <MDBCardBody className="mx-4">
                      <div className="text-left">
                        ...
                        <MDBRow className="mbottom0Imp">
                          <MDBCol md="10">
                            <h5>
                              <strong>SECTION 7. Student Training Plan</strong>
                            </h5>
                          </MDBCol>
                      ...
                      <div className="text-left">
                        ...
                        >
                          ...
                          <textarea
                            ...
                            rows="8"
                            defaultValue={this.state.getS7q1}
                            onChange={(e) => this.setState({ s7q1: e.target.value })}
                          />
                        </form>
                      </div>
                      {/* ----------- button group ----------- */}
