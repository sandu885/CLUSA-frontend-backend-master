...
class Section11 extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.userName === null) {
      ...
      console.warn('section 11 logedin', this.state.logedin);
    } else {
      axios.post(
        getSectionInputAPI,
        ...
        if (response.data.message === 'Successfully get application content') {
          console.warn('section 11 get information successfully!');
          this.setState({
            getS11q1: response.data.content['1'],
          });
        } else if (response.data.message === 'No saved application for this section') {
          console.warn('no saved history');
        } else {
          console.warn('section 11 Failed to save');
        }
  ...
  clickSaveBtn = () => {
    ...
    console.warn('============== section 11 save data', this.state);
    const finalS11q1 = this.state.s11q1 === null ? this.state.getS11q1 : this.state.s11q1;

    const formData = new FormData();
    formData.append('sessionToken', this.state.sessionToken);
    formData.append('programType', '0');
    formData.append('sectionIndex', '11');
    ...
    axios.post(
      ...
      if (response.data.message === 'Successfully save section content') {
        ...
        document.getElementById('internship-s11').reset();
        window.location.reload();
      } else {
        console.warn('section 11 Failed to save');
        alert('section 11 Failed to save, please try again');
      }
      // console.warn(this.state.responseMessage);
    }).catch((error) => {
      console.warn('section 11 error.response', error.response);
      if(error.response !== null && error.response !== undefined) {
        if( error.response.data !== null && error.response.data !== undefined ) {
          if (error.response.data.message === 'sessionToken expired' || error.response.data.message === 'No sessionToken') {
            localStorage.clear();
  ...
  render() {
    ...
    if (this.state.shouldRedirectToNext === true) return <Redirect to="/internship-application-section12" />;
    if (this.state.shouldRedirectToPrevious === true) return <Redirect to="/internship-application-section10" />;
                    ...
                    <MDBCardBody className="mx-4">
                      <div className="text-left">
                        ...
                        <MDBRow className="mbottom0Imp">
                          <MDBCol md="10">
                            <h5>
                              <strong>SECTION 11. Future Plan</strong>
                            </h5>
                          </MDBCol>
                      ...
                      <div className="text-left">
                        ...
                        >
                          ...
                          <textarea
                            id="internship-s11-q1"
                            className="form-control"
                            rows="10"
                            defaultValue={this.state.getS11q1}
                            onChange={(e) => this.setState({ s11q1: e.target.value })}
                          />
                        </form>
                      </div>
                      {/* ----------- button group ----------- */}
